...
export function CommandCenter({ overview, supervisor, counts }: { overview: Overview; supervisor: Supervisor; counts: DashboardCounts }) {
  ...
  const workload = supervisor.workload.slice(0, 6);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* KPIs operativos con tendencia */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 14 }}>
        <Kpi label={t("dash.backlog")} value={supervisor.open}
          sub={<span style={{ color: delta > 0 ? "var(--st-critical-fg)" : delta < 0 ? "var(--st-low-fg)" : "var(--muted)" }}>{delta > 0 ? "▲" : delta < 0 ? "▼" : "•"} {today} {t("dash.today")}</span>}
          spark={inflow} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <Panel title={t("dash.aging")}>
          {supervisor.open === 0 ? <Empty text={t("dash.opsempty")} /> : (
            <div style={{ display: "flex", alignItems: "flex-end", gap: 14, height: 118, paddingTop: 8 }}>
              {supervisor.aging.map((a) => (
                <div key={a.bucket} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, height: "100%", justifyContent: "flex-end" }}>
                  ...
                  <div style={{ width: "100%", maxWidth: 46, height: `${Math.max(4, (a.count / agingMax) * 82)}px`, background: AGING_COLOR[a.bucket], borderRadius: "5px 5px 0 0" }} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 4 }}>
              {funnel.map((f) => (
                <div key={f.key} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 12, color: "var(--text)", width: 82, flexShrink: 0 }}>{t(("st." + f.key) as MessageKey)}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9, paddingTop: 4 }}>
            {workload.map((w) => (
              <div key={w.agent} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 12.5, color: "var(--text)", width: 120, flexShrink: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{w.agent}</span>
                <div style={{ flex: 1, height: 8, borderRadius: 20, background: "var(--track)", overflow: "hidden" }}>
      ...
      {/* Inventario de plataforma (secundario) */}
      <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--muted)", marginTop: 6 }}>{t("dash.platform")}</div>
