...
// ENTIDADES (tickets, proyectos, clientes, articulos) filtrada por permiso, abrir por numero
// y recientes. La config de navegacion vive en lib/nav/navigation.ts (fuente unica).

type Kind = "action" | "nav" | "entity" | "recent";
type Entry = { id: string; label: string; hint: string; icon: string; path: string; kind: Kind };
...
export function CommandMenu({ perms = [], isAdmin = false }: { perms?: string[]; isAdmin?: boolean }) {
  ...
  // Navegacion + acciones (sincronas, sobre la config).
  ...
  useEffect(() => { if (openMenu) { setQ(""); setSel(0); setEntities([]); setRecents(loadRecents()); setTimeout(() => inputRef.current?.focus(), 0); } }, [openMenu]);
  ...
  function go(e: Entry) { setOpenMenu(false); if (e.kind === "nav" || e.kind === "entity" || e.kind === "recent") pushRecent(e); router.push(e.path); }
