...
export function AiKb({ incidentId }: { incidentId: string }) {
  ...
  if (saved) {
    return <div style={{ fontSize: 13, color: "var(--st-low-fg)" }}>✓ {t("kb.saved")} <b style={{ fontFamily: "var(--font-mono)" }}>{saved}</b></div>;
  }
  ...
  return content ? (
    ...
    <div>
      <button onClick={gen} disabled={busy}
        ...
        <span style={{ color: "var(--accent-bright)" }}>✦</span> {busy ? t("ai.generating") : t("kb.gen.button")}
      </button>
