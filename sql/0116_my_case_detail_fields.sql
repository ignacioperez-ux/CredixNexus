-- 0116_my_case_detail_fields.sql
-- Portal usuario (Fase B1): la vista de detalle del caso PROPIO muestra "Detalle del caso"
-- (aplicacion/servicio/producto/canal/unidad + reportado por). Extiende get_my_case (SECURITY
-- DEFINER, owner-checked) con esos campos NOMBRE (joins a los maestros). Solo lectura, del caso
-- propio (reported_by_user_id = current_account_id()). Aditivo y seguro; no expone datos de otros.
-- Cambia el RETURNS TABLE -> hay que DROP + CREATE (create or replace no permite cambiar el tipo).
...
returns table (
  ...
  app text, service text, product text, channel text, business_unit text, reporter text
)
language sql stable security definer set search_path = public, auth as $$
  select i.id, i.incident_number::text, i.title::text, i.description::text, i.status::text,
         ...
         coalesce(ua.full_name, ua.username, ua.email)::text
