...
export type SquadLoad = { id: string; name: string; capacity_points: number; allocatedPoints: number };
...
export async function getWorkload(supabase: SupabaseClient): Promise<Workload> {
  const [membersRes, incRes, taskRes, projRes, squadRes] = await Promise.all([
    ...
    supabase.from("squad").select("id, name, capacity_points").eq("status", "active"),
  ]);
  ...
  const squads = (squadRes.data ?? []) as { id: string; name: string; capacity_points: number }[];
  ...
  const memberLoad: MemberLoad[] = members.map((m) => {
    ...
    return { id: m.id, name: m.name, discipline: m.discipline, capacity_points: m.capacity_points, openIncidents, taskPoints, taskCount: openTasks.length };
  });
  ...
  const squadLoad: SquadLoad[] = squads.map((s) => {
    ...
    return { id: s.id, name: s.name, capacity_points: s.capacity_points, allocatedPoints };
  });
