...
export default async function SquadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [roster, assignable, canManage, leads, initiatives, caps] = await Promise.all([
  ...
  return <SquadDetail squad={squad as never} roster={roster} assignable={assignable} canManage={canManage} leads={leads} initiatives={initiatives} capacity={cap} />;
}
